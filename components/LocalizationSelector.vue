...
<template>
  ...
  <div class="flex gap-2 w-1/2 mx-auto">
    <USelectMenu color="mindaro" v-model="selectedCity" :options="cities" option-attribute="name" :loading="isLoading" placeholder="Filtrar por ciudad" size="lg" class="w-full" :ui-menu="{ container: 'z-[1500] group' }"/>
  ...
  <div class="flex gap-2 w-1/2 mx-auto" v-if="selectedCity">
      <USelectMenu color="mindaro" v-model="selectedSubdivision" :options="subdivisionsOptions" :loading="isLoading" placeholder="Filtrar por ubicación" size="lg" class="w-full" :ui-menu="{ container: 'z-[1500] group' }">
