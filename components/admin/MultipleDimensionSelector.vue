<script setup>
  ...
  watch(selectedDimensions, (newValue, oldValue) => {
    if(newValue.length > 2) {
      toast.add({ title: 'Error', description: 'Solo puedes seleccionar hasta 2 dimensiones', color: 'red' })
...
<template>
  <div>
    <div class="flex gap-2">
      <USelectMenu v-model="selectedDimensions" class="w-full" :options="dimensions" :loading="isLoading" placeholder="Seleccione una o dos dimensiones" multiple>
        ...
        <template #label>
          <span v-if="selectedDimensions.length">{{ selectedDimensions.length }} Dimensiones seleccionadas</span>
          <span v-else>Seleccione una o dos dimensiones</span>
        </template>
      </USelectMenu>
    ...
    <div v-if="props.showBadges" class="flex flex-wrap gap-2 mt-2 items-center">
      <p class="text-xs text-gray-400">Dimensiones</p>
