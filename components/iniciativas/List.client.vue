...
<template>
  <div>
    ...
    <div v-else>
      ...
      <p class="text-center text-sm my-3 font-medium">Filtrar por eje temático (Hasta 2)</p>
      ...
      <div v-if="data && data.rows && data.rows.length">
        <div v-for="row in data.rows" :key="`iniciativa-id-${row.id}`" class="flex items-center gap-4 rounded-xl my-2 px-5 py-3 border bg-gray-900 hover:bg-gray-800 border-slate-800 transition-colors duration-200">
          <div class="grow w-9/12">
            ...
            <p class="font-bold text-xl cursor-pointer hover:text-mindaro transition-colors duration-500"  @click="goToDetail(row.id)" >{{ row.name }}</p>
            <p class="text-sm text-gray-500 truncate">{{ row.description }}</p>
          </div>
