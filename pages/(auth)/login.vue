      ...
      <UCard>
        ...
        <UForm ref="theForm" :schema="schema" :state="state" class="space-y-4" @error="onError" @submit.prevent="handleLogin" >
          ...
          <UFormGroup name="password" label="Password" required>
            <UInput v-model="state.password" icon="i-heroicons-lock-closed" size="xl" type="password" placeholder="Ingrese su contraseña" />
            <template #help>
              <UIcon name="i-heroicons-information-circle" /> <ULink :to="{name: 'recoverPassword'}" class="text-sm" >¿Olvidaste tú contraseña?</ULink>
            </template>
          </UFormGroup>
          <div class="text-right">
            <UButton color="primary" variant="soft" type="submit" icon="i-heroicons-paper-airplane" size="xl" block trailing :ui="{ rounded: 'rounded-full' }">Iniciar sesión</UButton>
          </div>
        </UForm>
      </UCard>
