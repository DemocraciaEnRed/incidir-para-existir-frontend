...
const { data, refresh, status } = useAPI('/blog/category', {
  server: false,
  query: {
    page,
    limit: pageCount,
  ...
  onResponse({ request, response, options }) {
    categories.value = response._data.rows
    totalCategories.value = response._data.count
  }
})
...
const updateCategory = async (category, name) => {
  try {
    await $api(`/blog/category/${category.id}`, {
      method: 'PUT',
      body: {
        name: name
      }
...
<template>
  <div>
    ...
    <p>Las categorias de las entradas del blog de Actualidad son categorias que se comparten entre las diferentes secciones del blog. Puede crear y editar las categorias existentes. Sin embargo, es requerido de que haya al menos una categoria en el sistema.</p>
    <UDivider class="my-4" />
